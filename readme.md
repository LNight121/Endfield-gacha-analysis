...
然后考虑转移，如果当前的大保底计数为 $i$ ，小保底计数为 $j$ ，记此时出UP的概率为 $x$ ，出六星但是歪的概率为 $y$ ,没出的概率为 $z$ ，那么 $dp_{i,j,k}=x*dp_{0,0,k+1}+y*dp_{i+1,0,k}+z*dp_{i+1,j+1,k}+1$ ，其中最后的+1是这次抽卡本身的花费。
...
然后考虑转移，有 $dp_{h,i,j,k}=x*dp_{h+1,0,0,k+1}+y*dp_{h+1,0,i+1,0,k}+z*dp_{h+1,i+1,j+1,k}+1$ 。
...
- 当 $k!=0$ 时， $a_{h,i,j,k}=x*a_{h+1,0,0,k-1}+y*a_{h+1,0,i+1,0,k-1}+z*a_{h+1,i+1,j+1,k-1}$ 。
- 当 $k==0$ 时， $a_{h,i,j,k}=x*a_{h+1,0,0,k-1}+y*a_{h+1,0,i+1,0,k-1}+z*a_{h+1,i+1,j+1,k-1} + 1$ 。
